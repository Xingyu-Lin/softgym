...
inline std::string GetFilePathByPlatform(const char* path)
{
	...
	static bool resourceFound = false;
	if (!resourceFound)
	{
		const char* toTry[] = 
		{
#ifndef WIN32
			"FlexRobotics/demo/scenes/",
			"demo/scenes/",
			"scenes/",
#else
			"demo\\scenes\\",
		...
		for (const char** t = toTry; *t; t++)
		{
			struct stat statbuf;
			bool success;
			std::string test = std::string(*t) + path;
			success = 0 == stat(test.c_str(), &statbuf);
			if (success)
			{
				resourcePath = *t;
				resourceFound = true;
	...
	std::string newPath(resourcePath + path);
