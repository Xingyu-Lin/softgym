...
//mostly taken from Real Time Collision Detection - p192
CUDA_CALLABLE inline bool IntersectSegmentTri(const Vec3& p, const Vec3& q, const Vec3& a, const Vec3& b, const Vec3& c,  float& t, float& u, float& v, float& w, Vec3* normal)
{
	const Vec3 ab = b-a;
	const Vec3 ac = c-a;
	const Vec3 qp = p-q;
...
CUDA_CALLABLE inline Vec3 ClosestPointOnFatTriangle(const Vec3& a, const Vec3& b, const Vec3& c, const Vec3& p, const float thickness, float& v, float& w)
{
...
// computes intersection between a ray and a triangle expanded by a constant thickness, also works for ray-sphere and ray-capsule
// this is an iterative method similar to sphere tracing but for convex objects, see http://dtecta.com/papers/jgt04raycast.pdf
CUDA_CALLABLE inline bool IntersectRayFatTriangle(const Vec3& p, const Vec3& dir, const Vec3& a, const Vec3& b, const Vec3& c, float thickness, float threshold, float maxT, float& t, float& u, float& v, float& w, Vec3* normal)
{
	t = 0.0f;
	Vec3 x = p;
	Vec3 n;
...
CUDA_CALLABLE inline float ClosestPointBetweenLineSegmentAndTri(const Vec3& p, const Vec3& q, const Vec3& a, const Vec3& b, const Vec3& c, float& outT, float& outV, float& outW)
{
	...
	if (IntersectSegmentTri(p, q, a, b, c, t, u, v, w, NULL))
	{
		outT = t;
		outV = v;
		outW = w;
...
CUDA_CALLABLE inline bool IntersectRayAABBFast(const Vec3& pos, const Vector3& rcp_dir, const Vector3& min, const Vector3& max, float& t) {
...
{
	//Based on Fast Extraction of Viewing Frustum Planes from the WorldView-Projection Matrix, Gill Grib, Klaus Hartmann
