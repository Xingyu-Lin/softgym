...
{	
	...
	int* triangleFeatures;			//!< Feature assignment for each triangle
...
NV_FLEX_API void DynamicBVHTest(void);

struct NvFlexExtTetrahedralization
{
	float* vertices;
	int numVertices;
...
NV_FLEX_API NvFlexExtTetrahedralization* NvFlexExtCreateTetrahedralization(const float* points, int numPoints, const int* indices, int numTris);
NV_FLEX_API void NvFlexExtDestroyTetrahedralization(NvFlexExtTetrahedralization* t);


struct NvFlexExtMeshAdjacency
{
	int* triFeatures;		//!< Unique assignment of mesh features	to triangles (numTris in length), see NvFlexTriFeature

	int* vertexAdj;			//!< Vertex adjacency information (0-n vertices opposite each vertex, the 1-star)
	int* vertexAdjOffset;	//!< Offset in the adjacency array for each vertex (numVertices in length)
	int* vertexAdjCount;	//!< Number of neighboring vertices to each vertex (numVertices in length)
	
	int* edgeAdj;			//!< Edge adjacency information (0-1 vertices opposite edge, 3*numTris in length)

	int numVertices;		//!< Number of vertices
	int numVertexAdjs;		//!< Number of entries in the vertexAdj array
