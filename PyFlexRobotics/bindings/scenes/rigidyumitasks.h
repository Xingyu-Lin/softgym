...
class RigidYumiTasks : public Scene
{
public:
    /*
     * Yumi RL Tasks setup.
    ...
    using URDFPtr = std::shared_ptr<URDFImporter>;

    URDFImporter* robot_urdf;
    URDFImporter* cabinet_urdf;
    URDFImporter* blue_urdf;
    vector<URDFPtr> object_urdfs;
    ...
    float tableHeight = 0.39f;

    std::string red = "task_objects/red.urdf";
    std::string blue = "task_objects/blue.urdf";
    std::string yellow = "task_objects/yellow.urdf";
    std::string green = "task_objects/green.urdf";

    // Generate a random transform that is acceptable for a minor manipulable
    // object.
    Transform GetObjectBasePose() {

        Transform transform(Vec3(0.0f, 1.f * rand(), 1.f * rand()),
                            QuatFromAxisAngle(Vec3(0.0f, 1.0f, 0.0f),
                                              -kPi*0.5f));
        return transform;
    }

    // Randomly spawn the minor objects on the table top. This is used to
    // initialize their positions; it does not make any assumptions about where
    // they will go.
    void PlaceObjects(const int hiddenMaterial) {
        for (const auto& urdf: object_urdfs) {
            std::cout << "adding urdf\n";
            Transform transform = GetObjectBasePose();
            urdf->AddPhysicsEntities(transform, hiddenMaterial,
                                     true, 1000.0f, 0.0f, 1e1f, 0.01f, 20.7f,
                                     7.0f, false);
        }
    }
    ...
    void SetupCube() {
        cubeMasses.resize(numCubes);
        cubeScales.resize(numCubes);
        cubeShapeBodyIds.resize(numCubes);
        cubeGrasped.resize(numCubes);
    }
    ...
    RigidYumiTasks() {
        ...
        std::cout << "!!!!!!!!!!!!!!!!!!\n";
        std::cout << "Creating YuMi tasks environment";
...
#if 1
        object_urdfs.push_back(URDFPtr(new URDFImporter("../../data", red)));
        object_urdfs.push_back(URDFPtr(new URDFImporter("../../data", blue)));
        object_urdfs.push_back(URDFPtr(new URDFImporter("../../data", green)));
        object_urdfs.push_back(URDFPtr(new URDFImporter("../../data", yellow)));
#endif

        Transform robot_transform(Vec3(0.0f, 0.025f, 0.0f),
                                  QuatFromAxisAngle(Vec3(0.0f, 1.0f, 0.0f),
                                                    -kPi*0.5f) * QuatFromAxisAngle(Vec3(1.0f, 0.0f, 0.0f), -kPi*0.5f));
        Transform cabinet_transform(Vec3(0.0f, 0.025f, 0.3f),
                                    QuatFromAxisAngle(Vec3(0.0f, 1.0f, 0.0f),
                                                      -kPi*0.5f)*QuatFromAxisAngle(Vec3(1.0f, 0.0f, 0.0f), -kPi*0.5f));

        // hide collision shapes
        const int hiddenMaterial = AddRenderMaterial(0.0f, 0.0f, 0.0f, true);
        const int nonHiddenMaterial = AddRenderMaterial(0.0f, 0.0f, 0.0f, false);

        robot_urdf->AddPhysicsEntities(robot_transform, hiddenMaterial, true,
                                       1000.0f, 0.0f, 1e1f, 0.01f, 20.7f, 7.0f, false);


        std::cout << "=======================================================\n";
        Transform transform = GetObjectBasePose();
        blue_urdf = new URDFImporter("../../data", blue);
        blue_urdf->AddPhysicsEntities(transform, nonHiddenMaterial,
                                      true, 1000.0f, 0.0f, 1e1f, 0.01f, 20.7f,
                                      7.0f, false);
        std::cout << "=======================================================\n";
#if 0
        cabinet_urdf->AddPhysicsEntities(cabinet_transform, hiddenMaterial,
                                         true, 1000.0f, 0.0f, 1e1f, 0.01f, 20.7f,
                                         7.0f, false);
#endif
        PlaceObjects(nonHiddenMaterial);
    ...
    // AddTable
    // Helper function to create a table of this particular height.
    // TODO(cpaxton): make sure we can properly configure the position of the
    // table.
    void AddTable(float tableHeight) {
        NvFlexRigidShape table;
        NvFlexMakeRigidBoxShape(&table, -1, 0.55f, tableHeight, 0.27f,
                                NvFlexMakeRigidPose(Vec3(0.0f, 0.0f, 0.6f), Quat()));
        table.filter = 0x0;
        table.group = 0;
        table.material.friction = 0.7f;
