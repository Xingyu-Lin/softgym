...
#include <vector>
#include "../urdf.h"
class RigidYumiMultipleGrippers : public Scene
{
public:
    ...
    enum MOTION_PHASE { APPROACH, GRIP, LIFT, END };
    ...
    RigidYumiMultipleGrippers()	: 
		rigidContacts(g_flexLib, g_solverDesc.maxRigidBodyContacts),
        rigidContactCount(g_flexLib, 1)
    {
        gripperInitialWidth = 0.025f;
        gripperWidth = gripperInitialWidth;
        ...
        Transform gt(Vec3(0.0f, 0.3f, 0.0f), QuatFromAxisAngle(Vec3(0.0f, 1.0f, 0.0f), -kPi*0.5f)*QuatFromAxisAngle(Vec3(1.0f, 0.0f, 0.0f), -kPi*0.5f));
        ...
        NvFlexMakeFixedJoint(&effectorJoint0, -1, handRight.body0, NvFlexMakeRigidPose(Vec3(0.0f, 0.3f, 0.0f), Quat()), NvFlexMakeRigidPose(0, 0));
        ...
        lfingerIndex = urdf->rigidNameMap["gripper_r_finger_l"];
        rfingerIndex = urdf->rigidNameMap["gripper_r_finger_r"];
        ...
        */
        ...
        LoadSimpleOBJ("../../data/dex-net/mini_dexnet/bar_clamp.obj", Transform(Vec3(x, 0.05f, 0.0f)));
        objIndex = g_buffers->rigidBodies.size() - 1;
    ...
    virtual void DoGui()
    {
        ...
        bool robotHitFloor = false;
        
		NvFlexRigidContact* ct = &rigidContacts[0];
        float lImpulse = 0.0f;
        float rImpulse = 0.0f;
        float cImpulse = 0.0f; // Contact impulse
        float th = 0.9f;
        float hitThreshold = 0.01f; // N
        float gripThreshold = 0.2f; // N
        float dt = 1.0f / 60.0f;
        float gripperSqueezeSpeed = 0.0001f / dt; // m/s
        float gripperReleaseSpeed = 0.001f / dt; // m/sl
        float gripperMoveSpeed = 0.001f / dt; // m/s
        float gripperMinWidth = 0.01f; // m
        float gripperInitHeight = 0.4f;
        for (int i = 0; i < numContacts; ++i)
        {
            if (ct[i].lambda > 0)
            {
                if (((ct[i].body0 == rfingerIndex) || (ct[i].body0 == lfingerIndex)) && (ct[i].body1 == -1))
                {
                    robotHitFloor = true;
                }
                else if (((ct[i].body1 == rfingerIndex) || (ct[i].body1 == lfingerIndex)) && (ct[i].body0 == -1))
                {
                    robotHitFloor = true;
                }
                else if ((ct[i].body0 != -1) && (ct[i].body1 != -1))
                {
                    int other = 0;
                    Vec3 normal;
                    if (ct[i].body0 == objIndex)
                    {
                        other = ct[i].body1;
                        normal = Vec3(ct[i].normal);
                    }
                    else if (ct[i].body1 == objIndex)
                    {
                        other = ct[i].body0;
                        normal = -Vec3(ct[i].normal);
                    }
                    ...
                    if (other == lfingerIndex)
                    {
                        if (normal.x > th)
                        {
                            lImpulse += ct[i].lambda;
                        }
                        else
                        {
                            cImpulse += ct[i].lambda;
                        }
                    }
                    else if (other == rfingerIndex)
                    {
                        if (normal.x < -th)
                        {
                            rImpulse += ct[i].lambda;
                        }
                        else
                        {
                            cImpulse += ct[i].lambda;
        ...
        {
            if ((robotHitFloor) || (cImpulse > hitThreshold))
            {
                motionPhase = GRIP;
            }
            else
            {
                gripperWidth += gripperReleaseSpeed * dt;
                if (gripperWidth > gripperInitialWidth)
                {
                    gripperWidth = gripperInitialWidth;
                }
                effector0.pose0.p[1] -= gripperMoveSpeed * dt;
            }
        }
        else if (motionPhase == GRIP)
        {
            if ((rImpulse + lImpulse > gripThreshold) || (gripperWidth < gripperMinWidth))
            {
                motionPhase = LIFT;
            }
            else
            {
                gripperWidth -= gripperSqueezeSpeed * dt;
                if (gripperWidth < 0.0f)
                {
                    gripperWidth = 0.0f;
                }
            }
        }
        else if (motionPhase == LIFT)
        {
            effector0.pose0.p[1] += gripperMoveSpeed * dt;
            if (effector0.pose0.p[1] > gripperInitHeight)
            {
                motionPhase = APPROACH;
            }
        }
        ...
        imguiSlider("Gripper Z", &ePos.z, -0.5f, 0.5f, 0.001f);
