...
public:
	...
	//const float dilation = 0.001f;  // Orig
	//const float thickness = 0.001f; // Orig
	...
	DEXNetTest() : rigidContacts(g_flexLib, g_solverDesc.maxRigidBodyContacts), rigidContactCount(g_flexLib, 1)
	{		
		useVisualMesh = g_render;
		...
		frameSinceStart = 0;
		g_solverDesc.maxRigidBodyContacts = 256 * 256 * 16;
		rigidContacts.map();
		...
		g_params.contactRegularization = pow(10.0f, -3.5f);
		...
		g_sceneLower = Vec3(-0.25f, -0.25f, -0.25f);
		g_sceneUpper = Vec3(0.5f*24.0f, 0.5f, 0.5f*24.0f)*0.01f;
		g_camSpeed = 0.005f;
		...
		numCols = 23;		// Number of columns per row
		spacing = 0.5f;   // Spacing of each gripper experiment in m
		...
		//urdf = new URDFImporter("../../data/", "yumi_description/urdf/yumi_gripper_nov17_with_visual_soft_tip.urdf", true, dilation, thickness);
			...
			*/
			...
			for (size_t e = 0; e < experiments.size(); e++)
			{
				...
				for (int i = ge.objectShapeBegin; i < ge.objectShapeEnd; i++)
				{
					NvFlexRigidShape& shape = g_buffers->rigidShapes[i];
					shape.material.friction = 1.0f;
					shape.material.torsionFriction = 0.05f;
	...
	virtual void CenterCamera(void)
	{
		...
		g_camPos = Vec3(0.483092f, 0.334608f, 1.00474f);
		g_camAngle = Vec3(12.5629f, -0.247836f, 0.f);
	}
...
#endif
		/*
		for (int i = 0; i < (int)urdf->deformables.size(); i++)
		{
			UpdateDeformableMesh(urdf->deformables[i]);
		}
		*/
	...
	virtual void PreSimulation()
	{
		if (!g_pause || g_step)
		{
			...
			for (int s = 0; s < numSimulationStepsPerRenderFrame; s++)
			{
				...
				float gripperMoveSpeed = 0.2f; // m/s
				float gripperAcceleration = 0.01f; // m/s^2
				float gripperMinWidth = 0.02f; // m
				...
				if (frameSinceStart == 1)
				{
					for (size_t i = 0; i < experiments.size(); i++)
					{
						...
						{
							//cout << "set experiment " << i << " to bad" << endl;
							for (int s = experiments[i].objectShapeBegin; s < experiments[i].objectShapeEnd; s++)
							{
...
class RigidPoseTest: public Scene
{
public:
	const float dilation = 0.0001f;
	const float thickness = 0.0003f;
	...
	void LoadSimpleOBJ(const char* path, const Transform& transform, Vec3* mins = NULL, Vec3* maxs = NULL)
	{
		...
		shape.material.friction = 1.0f;
		shape.thickness = thickness;
		shape.user = UnionCast<void*>(AddRenderMaterial(Vec3(.7f, .7f, .5f)));
	...
	RigidPoseTest()
	{
		...
		string objName = "bar_clamp";
		char objPath[1000];
		Vec3 mins, maxs;
		sprintf(objPath, "../../data/dex-net/mini_dexnet/%s.obj", objName.c_str());

		Mesh* m = ImportMesh(objPath);

		Quat q;
		Vec3 axis;
		float mag;
		FILE* f = fopen("pose.txt", "rt");
		fscanf(f, "%f %f %f %f\n", &q.x, &q.y, &q.z, &q.w);
		fscanf(f, "%f %f %f %f\n", &axis.x, &axis.y, &axis.z, &mag);
		fclose(f);

		Transform trans = Transform(Vec3(0.0f, 0.0f, 0.0f), QuatFromAxisAngle(axis, mag)*q);
		for (size_t i = 0; i < m->m_positions.size(); i++)
		{
			Vec3 p = TransformPoint(trans, ((Vec3&)m->m_positions[i]));
			m->m_positions[i] = p;

			m->m_normals[i] = Rotate(trans.q, m->m_normals[i]);
		}

		ExportToObj("tmp.obj", *m);

		LoadSimpleOBJ(objPath, trans, &mins, &maxs);
		g_buffers->rigidBodies.back().com[1] -= mins.y;
