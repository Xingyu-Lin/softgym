...
public:
	...
	virtual void Sync()
	{
		...
		NvFlexSetDynamicTriangles(g_solver, g_buffers->triangles.buffer, g_buffers->triangleNormals.buffer, NULL, g_buffers->triangles.size() / 3);
	}
	...
	virtual void Draw(int pass)
	{
		...
		for (size_t i = 0; i < mCloths.size(); ++i)
		{
			DrawCloth(&g_buffers->positions[0], &g_buffers->normals[0], NULL, &g_buffers->triangles[mCloths[i].triangleOffset], mCloths[i].asset->numTriangles, g_buffers->positions.size(), g_renderMaterials[(i + 2) % 6]);//, g_params.radius*0.25f);			
		}
	}
