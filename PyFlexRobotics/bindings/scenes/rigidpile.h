...
public:
	...
	RigidPile(int brickHeight) : mHeight(brickHeight)
	{
		int sx = 2;
		int sy = mHeight;
		int sz = 2;
			...
			for (int y=0; y < dimy; ++y)
				for (int z=0; z < dimz; ++z)
					for (int x=0; x < dimx; ++x)
						...
						g_params.radius*0.9f*Vec3(float(sx), float(sy), float(sz)), 0.0f, g_params.radius*0.9f, Vec3(0.0f), 1.0f, true, 1.0f, NvFlexMakePhase(g_buffers->shapeMatchingOffsets.size()+1, 0), true, 0.002f);// 0.002f);
		...
		else
		{
			...
			for (int y=0; y < wdimy; ++y)
			{
				for (int x=0; x < wdimx; ++x)
				{
					...
					//CreateParticleGrid(lower, bdimx, bdimy, bdimz, radius, Vec3(0.0f), 1.0f, true, g_buffers->shapeMatchingOffsets.size()+1, 0.0f);
					CreateParticleShape(GetFilePathByPlatform("../../data/box.ply").c_str(), lower + RandomUnitVector()*Vec3(0.0f, 0.0f, 0.0f), Vec3(bdimx*radius, bdimy*radius, bdimz*radius), 0.0f, radius, Vec3(0.0f), 1.0f, true, 1.0f, NvFlexMakePhase(g_buffers->shapeMatchingOffsets.size()+1, 0), true, 0.0f, Vec3(0.0f, 0.0f, y*0.0001f));
				}
			}
				...
				for (int y=0; y < dimy; ++y)
					for (int z=0; z < 1; ++z)
						for (int x=0; x < 1; ++x)
							...
							g_params.radius*Vec3(float(sx), float(sy), float(sz)), 0.0f, g_params.radius, Vec3(0.0f), 1.0f, true, 1.0f, NvFlexMakePhase(g_buffers->shapeMatchingOffsets.size()+1, 0), true, 0.0f);
