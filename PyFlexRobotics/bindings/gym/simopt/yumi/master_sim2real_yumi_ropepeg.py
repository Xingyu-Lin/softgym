...
def run_robot(pi, egm_robot, total_timesteps, max_ep_len, obj_track_fn,
    ...
    while True:
        ...
        # curr_state = curr_target
        curr_state = np.asarray(curr_state)

        if t % control_iter == 0:
            if control_t == total_timesteps:
                robot_go(egm_robot, init_position, curr_target)
            ...
            ob[10:13] = obj_track[3:6]
            ob[13:16] = prev_ob[10:13]
            prev_ob = ob.copy()
            print (str(control_t) + " OB REAL " + str(ob))
...
def run(num_iter, start_iter, start_from, cfg_env_source, cfg_env_source_init,
        ...
        iter_flag, run_rl_flag, run_simopt_flag, obj_track_fn, run_real):
...
if __name__ == '__main__':
    ...
    def obj_track_fn():
        ...
        translation = np.asarray([
            ...
            -0.144636, 0.136791, 0.725087,
            # -0.150710, 0.025, 0.557756,
            # -0.150710, 0.165173, 0.557756,
            float(msg_elems[3]), float(msg_elems[4]), float(msg_elems[5])])
        # return [-0.2, 0.5, 0.7]
        return translation
    ...
    run(args.num_iter, args.start_iter, args.start_from, args.cfg_env_source, args.cfg_env_source_init,
        ...
        iter_flag, run_rl_flag, run_simopt_flag, obj_track_fn, args.run_real)
