...
void WriteSceneResults(TimerTotals& totals)
{
	...
	printf("Scene: %s\n", g_sceneFactories[g_sceneIndex].mName);
	...
	g_benchmarkFile << "Scene: " << g_sceneFactories[g_sceneIndex].mName << std::endl;
...
int GoToNextScene()
{
	...
	{
		if (strcmp(benchmarkList[g_benchmarkScene], g_sceneFactories[i].mName) == 0)
		{
			sceneToSwitchTo = i;
