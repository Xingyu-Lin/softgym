...
struct NvFlexExtContainer
{
	...
	NvFlexVector<Vec3> mTriangleFeatures;
	...
	NvFlexExtContainer(NvFlexLibrary* l) :
		...
		mSpringCoefficients(l),mTriangleIndices(l),mTriangleNormals(l), mTriangleFeatures(l),
...
void CompactObjects(NvFlexExtContainer* c)
{
	...
	c->mTriangleFeatures.resize(totalNumTris);
	...
	//-------------------------------
	// build unique triangle features, todo: this is slow, do it at asset creation time

	if (totalNumTris)
	{
		c->mParticles.map();

		NvFlexExtMeshAdjacency* triAdj = NvFlexExtCreateMeshAdjacency((float*)&c->mParticles[0], c->mParticles.size(), sizeof(Vec4), &c->mTriangleIndices[0], totalNumTris, false);
		memcpy(&c->mTriangleFeatures[0], triAdj->triFeatures, sizeof(int)*totalNumTris);
		NvFlexExtDestroyMeshAdjacency(triAdj);
	...
	if (c->mTriangleIndices.size())
		NvFlexSetDynamicTriangles(c->mSolver, c->mTriangleIndices.buffer, NULL, c->mTriangleFeatures.buffer, int(c->mTriangleIndices.size()/3));
	else
		NvFlexSetDynamicTriangles(c->mSolver, NULL, NULL, NULL, 0);
