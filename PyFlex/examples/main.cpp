...
/*
void RenderParticles(Vec4* particles, Vec3* velocities, int* phases, int numParticles) {
    printf("Position: %f, %f, %f ", particles[0].x, particles[0].y, particles[0].z);
    printf("Velocities: %f, %f, %f\n", velocities[0].x, velocities[0].y, velocities[0].z);
}

Vec4 RandomSpawnPosition() {
    return Randf(Vec4(1, 1, 1, 1));
}

Vec3 RandomSpawnVelocity() {
...
int main() {
    ...
    NvFlexBuffer* velocityBuffer = NvFlexAllocBuffer(library, n, sizeof(Vec3), eNvFlexBufferHost);
    NvFlexBuffer* phaseBuffer = NvFlexAllocBuffer(library, n, sizeof(int), eNvFlexBufferHost);

    // map buffers for reading / writing
    auto particles = (Vec4*)NvFlexMap(particleBuffer, eNvFlexMapWait);
    auto velocities = (Vec3*)NvFlexMap(velocityBuffer, eNvFlexMapWait);
    ...
    for (int i = 0; i < 100; i++) {
        // map buffers for reading / writing
        particles = (Vec4*)NvFlexMap(particleBuffer, eNvFlexMapWait);
        velocities = (Vec3*)NvFlexMap(velocityBuffer, eNvFlexMapWait);
